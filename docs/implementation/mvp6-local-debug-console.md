...
routing path. It is a Python localhost server plus plain HTML/JS page for
recording one microphone draft, explicitly running that draft through the
routing chain, inspecting metadata-only pipeline results, and saving local-only
QA history.
...
- MVP6-PIPELINE-INSPECTOR-001: page displays local_audio_gate, asr, thinker, router, and qa_history stages.
- MVP6-QA-HISTORY-001: local QA history stores ASR question text and debug route answer by default.
