...
- TTS adapter contract event: `TTS_SYNTHESIS_OUTPUT_EMITTED` for normalized playback-compatible audio refs and metadata. Replay uses recorded refs only; it does not rerun the TTS provider, does not require raw audio, and missing truncate capability must be explicit degraded/blocking metadata rather than a silent barge-in target validation pass.
